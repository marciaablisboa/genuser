...
error_reporting(E_ERROR | E_WARNING | E_PARSE);
	...
	if(isset($_POST['confirmar'])){
		...
		// 1 - Registro dos dados em uma Sessão //garantir que dados já digitados não sejam perdidos
		...
		if(substr_count($_SESSION['email'], '@') != 1 || substr_count($_SESSION['email'], '.') < 1 || substr_count($_SESSION['email'], '.') > 3)
		...
		if(count($erro) == 0){
			...
			if($confirma){
				unset($_SESSION[nome], //deletar todas as variáveis criadas na sessão, depois q a inserção for feita com sucesso
				$_SESSION[sobrenome],
				$_SESSION[email],
				$_SESSION[senha],
				$_SESSION[sexo],
